...
var videoExtractor={
  // index:index,
   extractVideoByTheme:extractVideoByTheme
   //mongoConnectInitial:mongoConnectInitial
};


/* the function index is no longer used.
function index(){
...
function extractVideoByTheme(queryStr,youtube){
   ...
   var videoInfo;
   var videoIdArray;
   videoIdArrayPromise=getVideoId(queryStr,youtube);

   //console.log('youtube in videoByThemm is not existed');
     ...
     for(var i=0;i<videoIdArray.length;i++){
      // console.log("videoIdArray: ",videoIdArray[i]);
        ...
        youtube.getById(videoIdArray[i],function(err,videoInfo){
          ...
          var data=getData(videoInfo);
          if(!data) {
              console.log("problems in getData(),nothing returned");return null;
          }
          MongoClient.connect(dbUrl, function(err, db) {
            if (err) throw err ;
            //console.log('sss debug:', 'mongodb videoinfos connnected');
            var mongoDb = db;
            var mongoTable = db.collection('videoinfos');
            insert2Mongo(data,mongoTable);

            console.log("this is after insert");
...
function  getVideoId(queryStr,youtube){
 return new Promise(function(resolve,reject){
  if(!queryStr) return null;
  ...
  if(!youtube){
    console.log('getVideoID: youtube not exists');
    return null;
  }
  //youtube.clearParams();
  ...
  youtube.addParam('relevanceLanguage',queryStr.relevanceLanguage);
  if(queryStr.query.regionCode)
  youtube.addParam('regionCode',queryStr.regionCode);
  if(queryStr.query.order)
  youtube.addParam('order',queryStr.order);
  if(!queryStr.query.q) return null;
  //console.log('videoInfoHindi.js/params is :', youtube.params);
  var videoIdArray=[];
  //console.log(youtube);
  console.log('searchWord: ',queryStr.query.q);
  youtube.search(queryStr.query.q,maxIDCount,function(error,search_items){
    if(error){
  	  return error;
    }
    if(!search_items){
...
//console.log(data);
   ...
   var currentTime = (new Date()).getTime();
    ...
    mongoTable.findOne(query, function(err, item) {
        if (err) {
            console.log("errors in mongo/findOne and later data will be inserted");
            callback();
            throw err;
